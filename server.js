/**
 * WISE OS UNIFIED — server.js v3.2.1
 * Corrections : Pool MySQL2 + Logs améliorés + Endpoint /status
 */
...
async function connectWhatsApp(tenantId) {
  ...
  try {
    ...
    const sock = makeWASocket({
      ...
      logger: { level: 'silent' },           // Réduit le bruit des logs Baileys
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      if (qr) {
        ...
        console.log(`[WA] 📱 QR Code généré pour tenant ${tenantId}`);
      }
...
async function startServer() {
  ...
  // Health Check
  app.get("/health", (_, res) => res.json({ status: "ok", version: "3.2.1" }));

  // === NOUVEL ENDPOINT STATUS ===
  app.get("/status", auth, (req, res) => {
    const sessionList = {};
    
    sessions.forEach((sd, tenantId) => {
  ...
  app.post("/generate-otp", auth, async (req, res) => {
    ...
    try {
      ...
      // sendWA reste à implémenter selon ton i18n
      console.log(`[OTP] Code généré pour ${phone} (tenant ${tenant_id})`);
    } catch (e) {
      console.error(e);
  ...
  app.listen(PORT, async () => {
    console.log(`🚀 Wise OS Unified v3.2.1 démarré sur port ${PORT}`);
    console.log(`📊 Endpoint status disponible : /status`);
...
// Keep-alive
setInterval(() => {
  console.log(`[KEEP-ALIVE] Wise OS v3.2.1 • ${new Date().toISOString()}`);
}, 4 * 60 * 1000);
