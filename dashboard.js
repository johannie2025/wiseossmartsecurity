 ...
 * Baileys + QR Code + Nodemailer + Toutes les routes + SSE
 * Mode Temporaire Render (stockage local)
 */
...
async function connectWhatsApp(tenantId) {
  ...
  try {
    console.log(`[WA ${tid}] Démarrage de la connexion...`);
    ...
    sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === "open") {
        ...
        console.log(`✅ WHATSAPP CONNECTÉ - Tenant ${tid}`);
      }
      if (connection === "close") {
...
async function startServer() {
  ...
  app.get("/health", (_, res) => res.json({ status: "ok", version: "3.3.5", mode: "full_temp" }));
  ...
  app.post("/validate-otp", async (req, res) => {
    res.json({ valid: true, note: "Mode temporaire - validation simulée" });
  });
  ...
  app.post("/send-magic", async (req, res) => {
    ...
    const html = `<h2>Bonjour ${name},</h2><p>Cliquez sur le lien pour vous connecter :</p><a href="${link}">Connexion Wise OS</a>`;
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Wise OS v3.3.5 FULL EXHAUSTIVE (Temporary Storage) démarré sur port ${PORT}`);
